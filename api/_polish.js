...
- tags: 이 생각과 관련된 성좌 키워드를 1~3개, 짧은 영문 소문자로(언어 무관, 하나만). 가능하면 아래 기존 키워드를 재사용하세요:
  bike, freedom, hands, pm, salt, trade, sales, timber, survival, commonsense, malaria,
  pakistan, islam, adapt, vienna, prague, croatia, cambodia, tokyo, shanghai, hangzhou, ireland,
  jinhae, masan, nomad, boundary, stranger, bridge, drift, interp, multiling, teaching, subtitle,
  guesthouse, bikery, english, nuance, culture, trust, shock, startup, navy, dog, war, winter,
  star, beer, mediation, repair, travel, translate, context, ecoventure, trash, plogging.
  **나라·도시처럼 너무 넓은 태그(china, korea, europe 등)는 피하세요** — 구체 지역이면 hangzhou/shanghai 등,
  특정 사건이면 그 사건을 나타내는 태그를 쓰세요.
  **복합 개념은 한 태그로 뭉개지 말고 단어 단위로 나누세요**
  (예: 쓰레기 줍기 하이킹 → 한 태그 'ecoventure'만 쓰지 말고 tags에 trash, plogging, ecoventure처럼 나눠 넣기 —
  최대 3개까지).
...
- tagLabels: tags에 넣은 태그 각각에 대해, 방문자에게 보여줄 표시 이름을 한국어/영어/중국어로 하나씩 주세요.
  **반드시 단어 하나로 — 성좌에 떠 있는 다른 단어들도 전부 한 단어입니다("통역", "자막", "PM", "영업", "손끝"처럼).
  ...
  (좋은 예: 위챗으로 사람을 모은 이야기 → tag: wechat, ko: "위챗", en: "WeChat", zh: "微信".
  쓰레기 줍기 하이킹 → tags: [trash, plogging, ecoventure],
    trash→ko:"쓰레기"/en:"Trash"/zh:"垃圾",
    plogging→ko:"플로깅"/en:"Plogging"/zh:"捡跑",
    ecoventure→ko:"에코벤처"/en:"Ecoventure"/zh:"生态".
  나쁜 예: "위챗을 통한 사람 모집", "쓰레기 줍기 하이킹", "捡垃圾徒步" — 문장·구·붙여쓴 복합어 금지).
