/* 인터뷰 원본 답변(질문+답)을 공개 사이트에 어울리는 절제된 1인칭 "생각" + 태그로 다듬는 공용 로직.
   interview-save.js(자동 발행)와 polish-thought.js(관리자 수동 재다듬기)가 함께 쓴다.
   ANTHROPIC_API_KEY 없거나 실패 시 원본 답변을 다듬지 않고 그대로 자르는 폴백으로 돌아간다. */
