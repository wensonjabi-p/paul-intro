...
방금 그가 인터뷰 질문 하나에 답했습니다. 두 가지를 만드세요:
...
const TOOL = {
  ...
  input_schema: {
    type: 'object',
    properties: {
      confirm: { type: 'string', description: '방금 답변을 한 문장으로 되짚어 확인하는 문구, 한국어' },
    ...
    required: ['confirm', 'question', 'tag', 'tier', 'choices'],
  },
};
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const { confirm, question: q, tag, tier, choices } = block.input;
    ...
    res.status(200).json({ followup: { confirm: confirm || null, q, node: tag || node || 'ai', tier: tier || 'deep', choices } });
  } catch (e) {
