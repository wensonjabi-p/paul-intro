...
// "카드 스타일 다시 만들기"에서 쓰는 재해석 모드 — 이미 있는 사실 기반 이야기를 훨씬 더 상상력 있게,
// 감각적인 디테일과 장면을 더해 다시 쓴다. 결과는 카드의 "사실 기반 %"를 낮춰서(클라이언트에서 처리) 표시하므로
// 여기서는 과감하게 상상해도 된다 — Paul 피드백: "세부내용까지 컨텍스트에 맞게 생성해도 돼, 사실기반 %만 낮아지면 돼".
const RESTYLE_SYSTEM = `당신은 Paul Bhang이라는 사람의 인생 이야기를 그의 목소리로 다시 쓰는 대필 작가입니다.
그는 한국인으로, 15년 넘게 5개 대륙을 오가며 살았습니다.

지금은 이미 있는 사실 기반 이야기 하나(또는 여러 개를 엮은 것)를 훨씬 더 상상력 있게 다시 씁니다.
핵심 사실(장소, 사건, 그가 실제로 겪은 일)은 바꾸지 말고 유지하되, 그 순간의 감각(냄새·소리·날씨·주변 풍경), 속마음, 짧은 대화처럼
원문에 없던 디테일을 자유롭게 상상해서 덧붙이세요 — 이건 사실 기록이 아니라 창작적으로 각색한 버전임을 감안하고 과감하게 상상하세요.

- 반드시 1인칭("나는"), 3~6문장, 하나의 문단.
- 여러 단어가 주어지면 순서대로 나열하지 말고 하나의 장면/흐름으로 자연스럽게 엮으세요.
- 너무 뻔하거나 진부한 상상은 피하고, 읽는 사람이 "오, 이런 각도였구나" 싶은 신선한 디테일을 넣으세요.`;
...
module.exports = async (req, res) => {
  ...
  const { nodes, lang, mode } = body || {};
  const isRestyle = mode === 'restyle';
  const minNodes = isRestyle ? 1 : 2;
  ...
  const baseSystem = isRestyle ? RESTYLE_SYSTEM : SYSTEM;
  const systemWithLang = baseSystem + `\n\n- 반드시 ${langName}로만 답하세요. 번역하지 말고, 처음부터 그 언어로 직접 쓰세요. 절대 다른 언어를 섞지 마세요.`;

  const userMsg = isRestyle
    ? nodes.map(n => `[${n.label}] ${n.story}`).join('\n\n') +
      `\n\n위 이야기를 훨씬 더 상상력 있게, 새로운 감각적 디테일을 더해 다시 써주세요. 반드시 ${langName}로 답하세요.`
    : nodes.map(n => `[${n.label}] ${n.story}`).join('\n\n') +
