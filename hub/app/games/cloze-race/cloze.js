/**
 * Cloze Race — fill particles/endings � type or tap � soft pace bar (no punish).
...
(function () {
  const STR = {
    en: {
      ...
      lead: "Complete particles & endings. Tap a chip or type — soft race bar, never punished by time.",
      backGames: "← Games",
      backHub: "Hub",
      note: "Pace bar is optional feel only — empty bar does not fail the question.",
      fillPrompt: "Fill the blank",
      pace: "Pace",
      yourAnswer: "Or type",
      ...
      restart: "Play again",
      loadFail: "Could not load cloze bank.",
      empty: "Pick a chip or type first.",
      themeFilter: "Theme",
      themeAll: "All",
    ...
    ko: {
      eyebrow: "빈칸 채우기",
      title: "빈칸 채우기",
      lead: "조사·어미를 채우세요. 칩을 누르거나 입력 — 페이스 바는 가볍게, 시간 처벌 없음.",
      backGames: "← 게임",
      backHub: "허브",
      ...
      restart: "다시 하기",
      loadFail: "빈칸 뱅크를 불러오지 못했습니다.",
      empty: "칩을 고르거나 먼저 입력하세요.",
      themeFilter: "테마",
      themeAll: "전체",
    ...
    zh: {
      eyebrow: "填空赛跑",
      title: "填空",
      lead: "补全助词与词尾。点选项或输入 — 进度条只是气氛，超时不判错。",
      backGames: "← 游戏",
      backHub: "中心",
  ...
  const BANK_URL = "../../data/games/cloze-beginner.json";
  const AUTO_KEY = "jabi.games.cloze.autoNext";
  const THEME_KEY = "jabi.games.cloze.theme";
  ...
  const THEME_ORDER = [
    ...
    "bathroom",
    "jobs",
    "country", "routine", "size", "senses", "color", "accessories", "electric", "building", "motion", "favor", "think", "speech", "change", "compare", "reason", "problem", "opinion", "habit", "rules", "friends", "personality", "apology", "success", "advice", "encourage", "promise", "refuse", "complain", "intermediate",
  ];
  ...
  function applyDic() {
    document.querySelectorAll("[data-clz]").forEach((el) => {
      const key = el.getAttribute("data-clz");
  ...
  function renderThemeChips() {
    ...
    ids.forEach((id) => {
      ...
      btn.type = "button";
      btn.className = "cloze-theme-chip" + (activeTheme === id ? " is-on" : "");
  ...
  function startRace() {
    ...
    raceStart = Date.now();
    raceLeft = 1;
    const fill = $("cloze-race-fill");
    const label = $("cloze-race-label");
    if (fill) {
      fill.classList.remove("is-draining");
      fill.style.width = "100%";
      // force reflow then drain
  ...
  function renderSentence(item, fillText, stateClass) {
    ...
    for (let i = 0; i < parts.length; i++) {
      if (i % 2 === 0) {
        ...
        const ans = item.answers?.[slot] || "";
        const shown = fillText != null ? fillText : selected || "���";
        const cls = ["cloze-blank"];
        if (fillText != null || selected) cls.push("is-filled");
        if (stateClass) cls.push(stateClass);
        const display = fillText != null ? (fillText || ans) : shown;
        html += `<span class="${cls.join(" ")}" data-slot="${slot}">${escapeHtml(display)}</span>`;
      }
    }
    el.innerHTML = html;
  }
  ...
  function renderChips(item) {
    const box = $("cloze-chips");
    if (!box) return;
    box.innerHTML = "";
    const choices = shuffleInPlace((item.choices || item.answers || []).slice());
    choices.forEach((c) => {
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "cloze-chip" + (selected === c ? " is-selected" : "");
      btn.textContent = c;
      ...
      btn.addEventListener("click", () => {
        ...
        renderSentence(item);
        box.querySelectorAll(".cloze-chip").forEach((el) => {
          el.classList.toggle("is-selected", el.textContent === c);
        });
      });
      box.appendChild(btn);
    });
  ...
  function updateProgress() {
    ...
    const text = $("cloze-progress-text");
    const fill = $("cloze-progress-fill");
  ...
  function showItem() {
    ...
    selected = "";
    const item = current();
    const stage = document.querySelector(".cloze-stage");
  ...
  function getUserAnswer() {
    const typed = ($("cloze-input")?.value || "").trim();
  ...
  function answersMatch(user, item) {
    ...
    // Single primary blank for MVP; join multi-blank if ever needed
    if (answers.length === 1) {
      return answers.some((a) => normalize(a) === u);
    }
    // If user typed full sentence
    if (normalize(item.full) === u) return true;
    const joined = answers.map(normalize).join("");
    return joined === u;
  }
  ...
  function showFeedback(kind, userRaw) {
    ...
    $("cloze-chips")?.querySelectorAll(".cloze-chip").forEach((el) => {
      el.disabled = true;
    });
    const input = $("cloze-input");
  ...
  function checkAnswer() {
    ...
    const raw = getUserAnswer();
    if (!raw) {
      toast(s("empty"));
      return;
    }
    const ok = answersMatch(raw, item);
    if (ok) {
      wasFast = raceLeft > 0.08;
      itemResult = "ok";
      showFeedback("ok", raw);
      const auto = $("cloze-auto-next");
      if (auto?.checked) {
  ...
  function reveal() {
    ...
    itemResult = "miss";
    showFeedback("reveal", getUserAnswer());
  }
  ...
  function bind() {
    $("cloze-check")?.addEventListener("click", checkAnswer);
    $("cloze-skip")?.addEventListener("click", skip);
    $("cloze-reveal")?.addEventListener("click", reveal);
    $("cloze-next")?.addEventListener("click", goNext);
    $("cloze-retry")?.addEventListener("click", retry);
    $("cloze-restart")?.addEventListener("click", restart);
    ...
    $("cloze-input")?.addEventListener("input", () => {
      if (checked) return;
      selected = ($("cloze-input").value || "").trim();
      const item = current();
      if (item) renderSentence(item);
      $("cloze-chips")?.querySelectorAll(".cloze-chip").forEach((el) => {
        el.classList.toggle("is-selected", el.textContent === selected);
      });
    });
  ...
  async function boot() {
    ...
    try {
      ...
      bankItems = (data.items || []).filter((it) => it.id && Array.isArray(it.answers));
