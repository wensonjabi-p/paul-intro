/**
 * Word Scramble � tap scrambled tokens to rebuild Korean word order.
 * Soft pace bar � never punished by time � original beginner bank.
 */
(function () {
  const STR = {
    en: {
      ...
      title: "Tap the order",
      lead: "Scrambled Korean chunks — tap to rebuild the sentence. Soft pace bar, never punished by time.",
      backGames: "← Games",
      backHub: "Hub",
      note: "Pace bar is feel only — empty bar does not fail the question. Tap a built token to undo.",
      buildPrompt: "Build the sentence",
      pace: "Pace",
      answerHint: "Tap tokens below →",
      poolLabel: "Tokens",
      check: "Check",
      clear: "Clear",
      skip: "Skip",
      ...
      restart: "Play again",
      loadFail: "Could not load scramble bank.",
      empty: "Tap tokens to build a sentence first.",
      dash: "—",
      themeFilter: "Theme",
      themeAll: "All",
    ...
    ko: {
      eyebrow: "글자 섞기",
      title: "순서 맞추기",
      lead: "섞인 한국어 조각을 탭해 문장을 만드세요. 페이스 바는 가볍게, 시간 처벌 없음.",
      backGames: "← 게임",
      backHub: "허브",
      note: "페이스 바는 분위기용 — 비어도 오답 처리하지 않습니다. 만든 조각을 탭하면 취소.",
      buildPrompt: "문장을 만드세요",
      pace: "페이스",
      answerHint: "아래 조각을 탭 →",
      poolLabel: "조각",
      check: "확인",
      ...
      restart: "다시 하기",
      loadFail: "글자 섞기 뱅크를 불러오지 못했습니다.",
      empty: "조각을 탭해 문장을 먼저 만드세요.",
      dash: "—",
      themeFilter: "테마",
      themeAll: "전체",
    ...
    zh: {
      eyebrow: "拼句",
      title: "点选顺序",
      lead: "打乱的韩语片段 — 点选重组句子。进度条只是气氛，超时不判错。",
      backGames: "← 游戏",
      backHub: "中心",
      note: "进度条仅氛围 — 空了也不判错。点已选片段可撤销。",
      buildPrompt: "组成句子",
      pace: "节奏",
      answerHint: "点下方片段 →",
      poolLabel: "片段",
  ...
  const BANK_URL = "../../data/games/word-scramble-beginner.json";
  const AUTO_KEY = "jabi.games.wordScramble.autoNext";
  const THEME_KEY = "jabi.games.wordScramble.theme";
  ...
  let poolOrder = [];
  /** @type {string[]} answer slot ids into poolOrder */
  let builtIds = [];
  let raceTimer = null;
  let raceStart = 0;
  let raceLeft = 1;
  ...
  function renderThemeChips() {
    ...
    ids.forEach((id) => {
      ...
      btn.type = "button";
      btn.className = "ws-theme-chip" + (activeTheme === id ? " is-on" : "");
  ...
  function startRace() {
    ...
    raceStart = Date.now();
    raceLeft = 1;
    const fill = $("ws-race-fill");
    const label = $("ws-race-label");
    if (fill) {
  ...
  function builtText() {
    return builtIds
      .map((id) => poolOrder.find((t) => t.id === id)?.text || "")
      .filter(Boolean)
      .join(" ");
  }
  ...
  function scrambleTokens(tokens) {
    const correct = tokens.slice();
    let scrambled = shuffleInPlace(tokens.slice());
    let guard = 0;
    while (tokensEqual(scrambled, correct) && scrambled.length > 1 && guard < 12) {
      scrambled = shuffleInPlace(tokens.slice());
      guard += 1;
    }
    return scrambled;
  }
  ...
  function renderBoard(mark) {
    const answerEl = $("ws-answer");
    const poolEl = $("ws-pool");
    const hint = $("ws-answer-hint");
    if (!answerEl || !poolEl) return;

    answerEl.innerHTML = "";
    answerEl.classList.remove("is-ok", "is-bad");
    if (mark === "ok") answerEl.classList.add("is-ok");
    if (mark === "bad") answerEl.classList.add("is-bad");

    builtIds.forEach((id, i) => {
      const tok = poolOrder.find((t) => t.id === id);
      if (!tok) return;
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "ws-token is-built";
      btn.setAttribute("role", "listitem");
      if (mark === "ok") btn.classList.add("is-ok");
      if (mark === "bad") btn.classList.add("is-bad");
      btn.disabled = checked;
      btn.innerHTML = escapeHtml(tok.text);
      btn.addEventListener("click", () => removeBuilt(i));
      answerEl.appendChild(btn);
    });

    if (hint) {
      hint.classList.toggle("is-hidden", builtIds.length > 0);
      hint.textContent = s("answerHint");
    }

    poolEl.innerHTML = "";
    poolOrder.forEach((tok) => {
      const used = builtIds.includes(tok.id);
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "ws-token" + (used ? " is-used" : "");
      btn.disabled = checked || used;
      btn.innerHTML = escapeHtml(tok.text);
      btn.addEventListener("click", () => addBuilt(tok.id));
      poolEl.appendChild(btn);
    });
  }

  function addBuilt(id) {
    if (checked || builtIds.includes(id)) return;
    builtIds.push(id);
    renderBoard(false);
  }

  function removeBuilt(index) {
    if (checked) return;
    if (index < 0 || index >= builtIds.length) return;
    builtIds.splice(index, 1);
    renderBoard(false);
  }
  ...
  function showFeedback(kind) {
    ...
    const status = $("ws-feedback-status");
    const yours = $("ws-yours");
    const correct = $("ws-correct");
    if (!fb) return;
    ...
    if (yours) yours.textContent = builtText() || s("dash");
    if (correct) correct.textContent = item.full || (item.tokens || []).join(" ");
    renderBoard(kind === "ok" ? "ok" : "bad");
  }
  ...
  function check() {
    ...
    const item = current();
    const mine = builtIds.map((id) => poolOrder.find((t) => t.id === id)?.text || "");
    const ok = tokensEqual(mine, item.tokens || []);
    wasFast = raceLeft > 0.08;
    itemResult = ok ? "ok" : "miss";
    showFeedback(ok ? "ok" : "miss");
    if (ok) {
      const auto = $("ws-auto-next");
  ...
  function prepareItem(item) {
    ...
    const scrambled = scrambleTokens(tokens);
    poolOrder = scrambled.map((text, i) => ({ text, id: `t${i}-${text}` }));
    builtIds = [];
  }
  ...
  function reveal() {
    ...
    const item = current();
    const used = new Set();
    builtIds = (item.tokens || [])
      .map((text) => {
        const found = poolOrder.find((t) => t.text === text && !used.has(t.id));
        if (found) {
          used.add(found.id);
          return found.id;
        }
  ...
  function bind() {
    ...
    $("ws-clear")?.addEventListener("click", clearBuilt);
    $("ws-skip")?.addEventListener("click", skip);
    $("ws-reveal")?.addEventListener("click", reveal);
    $("ws-next")?.addEventListener("click", goNext);
    $("ws-retry")?.addEventListener("click", retry);
    $("ws-restart")?.addEventListener("click", restart);
    ...
    document.querySelectorAll(".lang-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        renderItemTags(item);
        const hint = $("ws-answer-hint");
        if (hint) hint.textContent = s("answerHint");
        const label = $("ws-race-label");
        if (label && !checked) {
          if (raceLeft <= 0) {
            label.textContent = s("calm");
    ...
    document.addEventListener("keydown", (e) => {
      ...
      if (!checked && (e.key === "Backspace" || e.key === "Delete")) {
        if (builtIds.length) {
          e.preventDefault();
          removeBuilt(builtIds.length - 1);
        }
      }
    });
  }
  ...
  async function boot() {
    ...
    try {
      ...
      bankItems = (data.items || []).filter((it) => it.id && Array.isArray(it.tokens) && it.tokens.length);
