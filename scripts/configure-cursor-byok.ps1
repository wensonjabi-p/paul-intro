# Configure Cursor BYOK for Kimi (Moonshot) / DeepSeek on Windows.
# Usage:
#   .\scripts\configure-cursor-byok.ps1 -Provider kimi
#   .\scripts\configure-cursor-byok.ps1 -Provider deepseek

$ErrorActionPreference = "Stop"
$Provider = "kimi"
$ProviderOpt = @("kimi","deepseek")
param(
  [Parameter(Mandatory=$false)]
  [ValidateSet($ProviderOpt)]
  [string]$Provider
)
$ScriptDir = $PSScriptRoot
$KeysFile = Join-Path $ScriptDir "llm-keys.local.ps1"
$StateDb = Join-Path $env:APPDATA "Cursor\User\globalStorage\state.vscdb"
$BackupDb = "$StateDb.bak-byok"
...
if (-not $env:$ApiKeyEnvName -or $env:$ApiKeyEnvName -like "sk-your-*") {
